\documentclass[a4paper,fontsize=12pt,headsepline,BCOR=18mm,DIV=13]{scrbook}

\usepackage[ruled,vlined,linesnumbered]{algorithm2e}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage[english]{babel}
\usepackage[style=ieee,citestyle=numeric-comp,maxnames=99,dashed=false]{biblatex}
\usepackage{booktabs}
\usepackage[T1]{fontenc}
\usepackage{graphicx}
\usepackage[pdftex,hidelinks,plainpages=false,pdfpagelabels,pdfusetitle]{hyperref}
\usepackage[utf8]{inputenc}
% csquotes must be included after inputenc
\usepackage{csquotes}
\usepackage{listings}
\usepackage{multirow}
\usepackage{microtype}
\usepackage{newtxtext}
\usepackage{newtxmath}
\usepackage[section]{placeins}
% Suppress warning about \float@addtolists
\usepackage{scrhack}
\usepackage[labelformat=simple,format=hang]{subcaption}
\usepackage{tabularx}
\RequirePackage[l2tabu, orthodox]{nag}

% KOMA-Script settings
\addtokomafont{dedication}{\normalsize\itshape}

% Hyphenations to avoid overfull boxes
\hyphenation{Yo-shi-yu-ki}
\hyphenation{Ya-su-hi-ro}
\hyphenation{Wa-ta-shi-ba}
\hyphenation{Su-su-mu}
\hyphenation{To-shi-yu-ki}
\hyphenation{Shi-mi-zu}

% Suppress warning about multiple PDFs with page group
\pdfsuppresswarningpagegroup=1

% Allow PDF 1.7 documents to be used in \includegraphics
\pdfminorversion=7

\graphicspath{{./img/}}

% listing
\lstset{%
  language={C},
  basicstyle={\small\ttfamily},%
  identifierstyle={\small\ttfamily},%
  commentstyle={\small\itshape},%
  keywordstyle={\small\bfseries},%
  ndkeywordstyle={\small\ttfamily},%
  stringstyle={\small\ttfamily},%
  frame={tb},%
  breaklines=true,%
  columns=[l]{fullflexible},%
  numbers=left,%
  numberstyle={\scriptsize},%
  stepnumber=1,%
  numbersep=1em,%
  lineskip=-0.5ex,%
  mathescape,%
  xleftmargin=2em,%
  framexleftmargin=1.5em,%
}

% algorithm2e
\SetNlSty{normalfont}{}{}

% Numbering of subfigures
\renewcommand{\thesubfigure}{~(\alph{subfigure})}

\addbibresource{references.bib}

\title{Programmable Interconnect Control Adaptive to Communication Pattern \\
of Applications}
\author{Keichi Takahashi}
\dedication{This work is dedicated to my parents and my wife}

\renewcommand{\baselinestretch}{1.30}

\begin{document}

\pagenumbering{roman}

\maketitle

\input{src/publications}
\input{src/summary}

\tableofcontents
\listoffigures

\newpage
\thispagestyle{empty}

\pagenumbering{arabic}

\include{src/chapter1}
\include{src/chapter2}
\include{src/chapter3}
\include{src/chapter4}
\include{src/chapter5}
\include{src/acknowledgements}

\printbibliography[heading=bibintoc,title={References}]

\end{document}
