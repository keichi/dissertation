...
MPI collectives require intensive communication among multiple computing node
pairs. However, while some computing node pairs communicate less, some pairs
...
redundant routes between computing nodes, packet flow generated from MPI
...
decreases available bandwidth between computing nodes. Ultimately, this
...
the background on all computing nodes. The SDN MPI library is not a
...
processes, but in this chapter, we assume each computing node runs only a
single MPI process.
...
Each computing node runs an LLDP (Link Layer Discovery
...
\item Responding to Address Resolution Protocol (ARP) requests from computing
    nodes to avoid broadcast storms
\item Routing of non-MPI traffic such as ICMP and SSH
\end{itemize}

This controller is developed on top  of Trema~\autocite{trema}, a framework
...
connects computing nodes, SDN-enabled switches and the SDN controller.
...
\begin{figure}
    ...
    \caption{Comparison of Execution Time of MPI\_Allreduce (8 Computing Nodes)}%
...
\begin{figure}
    ...
    \caption{Speedup of Proposed MPI\_Allreduce (8 Computing Nodes)}%
...
\begin{figure}
    ...
    \caption{Comparison of Execution Time of MPI\_Allreduce (16 Computing Nodes)}%
...
\begin{figure}
    ...
    \caption{Speedup of Proposed MPI\_Allreduce (16 Computing Nodes)}%
