...
MPI}~\autocites{Takahashi2014}{Dashdavaa2013}, a framework that incorporates
...
is to accelerate individual MPI functions by dynamically optimizing the packet
flow in the interconnect. Several MPI functions have been successfully
accelerated in the previous works so far. One of the core challenges in the
research of SDN-enhanced MPI is to develop algorithms to control the packet
flow in the interconnect depending on the MPI function called by the
application.
...
applications and interconnects. However, utilizing actual clusters to
...
not yet been seen because the research and development of dynamic
interconnects are still at their early stage. Third, network hardware such as
switches may not support measuring traffic in the interconnect with enough
high frequency and precision to obtain meaningful insights.
...
This chapter aims to realize an interconnect simulator specialized
...
research~\autocites{Schneider2009}{Tikir2009}{Hoefler2010}{Jo2015} have been
designed to study the behavior of static interconnects. Therefore, the packet
flow is controlled based on static routing algorithms in these simulators.

However, the packet flow needs to be dynamically controlled based on the
communication patterns of applications for simulating application-aware
dynamic interconnects. Based on this necessity, the requirements for the
interconnect simulator are described as follows:
...
\item
  ...
  compute nodes. Thus, some means and mechanism to analyze the traffic volume
  of point-to-point communication exchanged between the processes from
...
\item
  ...
  scheduling, node selection and process placement algorithms affect the
  distribution of packet flow. These parameters should be easily
  reconfigurable to allow users to perform a systematic investigation of
  diverse clusters.
\end{itemize}
...
solvers) exhibit an iterative nature. These applications spend most of
...
the traffic matrix is a good representation of the communication
characteristics of an iterative application despite the loss of temporal
order in the traffic matrix. In the future, trace
segmentation~\autocite{Alawneh2016} techniques can be applied on the
communication trace. This technique will segment a trace into multiple
communication phases, which could then be simulated individually to further
...
communication pattern when profiling an application that uses collective
communication functions (\emph{e.g.} MPI\_Bcast, MPI\_Allreduce and
MPI\_Reduce).
...
applications and performance analysis tools. By using PERUSE, application or
performance analysis tools register callback functions for each event of
interest. After that, the registered callback function is called each time
...
PFProf is designed to be provided in the form of a shared library, so that it
can be easily integrated into applications either when building the
application or running the application. The preferred way is to use run-time
integration, because it requires neither recompilation nor relinking of the
application. The \lstinline!LD_PRELOAD! environment variable is used to load
the shared library before the execution of the application.
...
the contrary, PFSim can handle dynamic routing algorithms that use
communication patterns of applications and interconnect usage to make routing
decisions.
