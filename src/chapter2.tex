\chapter{Toolset for Analyzing Packet Flow in Interconnects}\label{sec:ii}
...
\subsection{Comparison of PFProf against Conventional Profiler}%
...
MPI\_Allreduce, one of the collective communications. Then, every process
performs point-to-point communication with its neighboring process. We
executed this application with 128 processes.
...
receiver rank, respectively. There is a pattern along the diagonal, which is
generated from the point-to-point communication with the neighbor processes.
However, the underlying point-to-point communication of MPI\_Allreduce could
not be observed. Figure~\ref{fig:pfprof-comm-matrix} shows the communication
pattern obtained with PFProf. This communication pattern clearly reveals the
MPI communication generated from MPI\_Allreduce in addition to the
point-to-point communication. From these observations, we conclude that PFProf
