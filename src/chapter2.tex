...
each packet is precisely reproduced. Therefore, the communication time of
applications can be predicted accurately in exchange for long execution time
and large memory foot print. In contrast, flow-level simulation estimates the
steady state behavior of the interconnect and does not track individual
packets. Thus, the packet flow in the interconnect can be speedily estimated
compared to packet-level simulation. As a trade-off, predicting the
communication time of applications using flow-level simulation is challenging.
As described in Section~\ref{sec:ii-objective}, this research aims at
realizing a toolset for efficiently predicting the packet flow in the
interconnect under diverse configurations. Therefore, PFSim is based on
flow-level simulation for execution efficiency.

% シミュレータの構造
Figure~\ref{fig:pfsim-arch} shows the internal structure of PFSim. PFSim is
based on a discrete-event simulation model. Under this model, the simulation
is driven by events, which indicate a change in the internal state of the
simulator. Each event holds (1) type of the event, (2) time when the event
will occur, and (3) additional information. The event queue is a priority
queue that stores the events according to the time it will occur. Event
handlers are functions that are associated to a specific event type and
invoked when an event with the associated type occurs. The dispatcher manages
...
% イベントハンドラの説明
In PFSim, three event types exist: job-submitted, job-started and
job-finished. In the event handler for the job-submitted event,
% PFSimにおいて主要なイベントは，ジョブの投入，ジョブの開始，ジョブの終了の3種類
% である．ジョブ投入イベントに対応するイベントハンドラでは，まず，ジョブキューが
% 空であり，かつ，ジョブを実行するために十分な数の空き計算機があるか判定する．も
% しジョブが実行可能でなければ，ジョブをジョブキューへ追加し，イベントハンドラを
% 終了する．もしジョブが実行可能であれば，アルゴリズム\ref{alg:start-job}に示す
% ジョブ開始準備処理へ移る．ジョブ開始準備処理では，はじめに，ジョブに対して空き
% 計算機を割り当てる．次に，割り当てた計算機上へのプロセスの配置を決定し，プロセ
% スと計算機を紐付ける．最後にジョブ開始イベントをイベントキューへ挿入する．
...
\begin{algorithm}
    ...
    \ForEach{(\Proc, \Node) $\in$ \Mapping}{%
        Associate \Proc to \Node\;
    }
    Insert job-started event to job queue\;

    \caption{Job start}%
    \label{alg:start-job}
\end{algorithm}

% アルゴリズム\ref{alg:on-job-started}にジョブ開始イベントハンドラの処理概要を示
% す．はじめに，ジョブが実行するアプリケーションのトラフィックマトリックスを取得
% する．次に，トラフィックマトリックスに基づき，相互結合網のトラフィック負荷を更
% 新する．更新の手順は次の通りである．まず，送信プロセス・受信プロセスそれぞれが
% 実行されている計算機を得る．続いて，送信プロセスが実行されている計算機から，受
% 信プロセスが実行されている計算機までの経路を計算する．最後に，計算した経路に含
% まれる全てのリンクについて，プロセス間で送受信されるトラフィック量を，リンクの
% トラフィック負荷へ加算する．トラフィック負荷の更新処理が完了した後，ジョブ終了
% イベントをイベントキューへ挿入する．
...
\begin{algorithm}
    ...
    Insert job-finished event to job queue\;
...
The job finished
% ジョブ終了イベントハンドラでは，ジョブに割り当てられていた計算機を解放する．
% そして，ジョブキューにジョブが存在すれば，ジョブを1つ選び出す．このジョブを実
% 行するために十分な空き計算機が存在すれば，アルゴリズム\ref{alg:start-job}の
% ジョブ開始準備処理を行う．


The simulation scenario file defines various configurations for a simulation
run. This file defines the cluster configuration to use and the set of jobs.
An example of a simulation scenario file is shown in
Listing~\ref{lst:simulation-scenario}. The cluster configuration file defines
the topology of the interconnect, the capacity of links, and the number of
processing elements for each compute node. Moreover, algorithms that control
the execution and communication of jobs are specified as shown in
Table~\ref{tbl:simulator-algorithm}.
...
Listing~\ref{lst:simulation-scenario}, two node allocation algorithms
(algorithms $\to$ node\_selector), two process placement algorithms
...
\begin{figure}
    \centering
    \includegraphics[width=.8\linewidth]{bandwidth_overhead}
...
\begin{figure}
    \centering
    \includegraphics[width=.8\linewidth]{latency_overhead}
...
\begin{figure}
    \centering
    \includegraphics[width=.8\linewidth]{nas_cg_multi_congestion}
...
\begin{figure}
    \centering
    \includegraphics[width=.8\linewidth]{nersc_milc_multi_congestion}
