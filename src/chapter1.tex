...
Point-to-point communication is a communication between one sender and one
receiver. On the other hand, collective communication involves a group of
processes. Table~\ref{tbl:mpi-primitives} shows some representative examples
of MPI primitives.
...
of high-performance computer clusters. As shown in Fig.~\ref{fig:mpi-arch},
each interconnect technology requires the programmer to use a different set of
API with different programming models. MPI hides the this incoherence and
allows programmers to develop applications without forcing them to study the
detailed architecture or structure of the underlying network.
