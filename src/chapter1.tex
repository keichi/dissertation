...
for allocating compute nodes to a job and launching the job on the allocated
...
single cluster accommodates up to tens of thousands of compute nodes and
millions of cores nowadays.

% 相互結合網の重要性
The performance of communication between compute nodes over the interconnect
is essential to the scalability of the cluster. In general, compute nodes
need to frequently communicate with each other during the parallel computation
to exchange intermediate results. If the communication between compute nodes
...
and low latency between a large number of compute nodes. Today,
interconnects of state-of-the-art HPC systems connect tens of thousands of
compute nodes where the bandwidth between two compute nodes reaches over
...
users to start MPI processes on multiple compute nodes.
...
is inherently application-agnostic.
...
on the compute nodes~\autocite{Michelogiannakis2017,Hoefler2011,Choi2017}.
...
are proposed to embed the communication pattern graph onto the interconnect
topology.
...
% コントローラ
The OpenFlow controller is a software responsible for the control plane. It
...
OpenDaylight\cite{Medved2014} are among the most popular OpenFlow controller
...
physical cluster is time-consuming, we utilize simulation to facilitate the
analysis. PFAnalyzer is a pair of two tools: PFSim, an interconnect simulator
specialized for programmable interconnects, and PFProf, a profiler to collect
communication pattern from MPI applications. PFSim allows interconnect
researchers and designers to investigate congestion in the interconnect for an
arbitrary cluster configuration and a set of communication patterns collected
by PFProf. We evaluate the accuracy of the simulation results obtained from
PFSim and demonstrate how PFAnalyzer can be used to analyze the effect of
programmable interconnect control.

% MPI集団通信を高速化するパケットフロー制御アルゴリズムを配備可能な
% フレームワークを構築
Chapter~\ref{sec:iii} addresses the second challenge. Out of the
communication primitives provided by MPI, we focus on accelerating collective
communication because it consumes a significant fraction of the execution time
of applications. We propose a framework to accelerate MPI collectives by
dynamically controlling the packet flow in the interconnect. We integrate the
network programmability provided by Software-Defined Networking into MPI
collectives so that collectives are able to effectively utilize the bandwidth
of the interconnect. In particular, we aim to reduce the execution time of
...
Chapter~\ref{sec:v} concludes this dissertation and discusses future work.
